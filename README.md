...
| Skill | 一句话职责 | 典型触发语 |
|---|---|---|
| `cann-ops:scann-repo` | 扫描一个 CANN ops 仓，揪出用了 **950 硬件特性**（simt / hif8 / RegBase / cube+vector 融合）的算子，给出 markdown 清单 + 机读 JSON | "扫一下 ops-transformer 里的 950 算子" |
| `cann-ops:ops-test` | 对目标算子跑 **Phase 1-4**（build / install / run / pytest），支持仓间并发、仓内合并 build、单算子兜底，最后产出带真实日志、复现命令、特性归因的报告 | "对 ops-cv 跑 phase1" |
...
skill 会：检查 Python 依赖 → 校验仓里有 `docs/zh/op_list.md` → 确认 cube/vector API 白名单（已有则问你复用还是刷新，没有则对话式从 API 文档 PDF 抽取）→ 扫描并渲染报告到 `cann-ops-report/scann/ops-transformer/`。
...
| 你怎么说 | skill 怎么取算子 |
|---|---|
| "对 ops-transformer 跑 phase1" | 已扫过 → 问你「跑扫出来的 N 个」还是「自己指定」 |
| "跑 phase1，目标：flash_attention_score, grouped_matmul" | 直接用你列的算子，不需要扫描产物 |
| "并发跑 ops-transformer 和 ops-cv 的 950 算子" | 收集两个仓路径，启进程池仓间并发 |
| "对 ops-math 跑 phase1，清单见 my-ops.txt" | 读文件（`.json` / 一行一算子的纯文本都行） |
...
cube/vector API 白名单从 Ascend C API 文档（PDF）对话式抽取并按章节映射；PDF 升级会自动触发 SHA 校验提示。
