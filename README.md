...
<你的 CWD>/
└── cann-ops-report/
    ├── SUMMARY.md                ← 每轮跑完自动生成的跨仓简洁摘要（给人看）
    │
    ├── <repo>/                   ← 每个被测仓一个子目录（scann + test 同级）
    │   ├── scann/                    ← scann-repo 扫描产物
    ...
    │       ├── logs/<op>.phase{N}.{build,install,run}.log
    │       ├── failures/<op>.md          （失败诊断）
    │       ├── explorations/<op>.md      （P6 自主探索结论）
    │       └── PHASE{N}_FINAL_REPORT.md  （最终结构化报告）
    │
    ├── issues/                   ← report-issues + track-issues 共享（跨仓，不按仓分）
...
skill 会：检查 Python 依赖 → 校验仓里有 `docs/zh/op_list.md` → 扫描并渲染报告到 `cann-ops-report/ops-transformer/scann/`。
...
跑测前 skill 会先问你**仓路径**和 **SOC**，然后调度合并 build + 串行 install + 逐算子 run，产物写到 `cann-ops-report/<repo>/test/`。
