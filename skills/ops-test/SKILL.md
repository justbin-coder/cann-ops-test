...
**输入来源**：目标算子从 `CWD/950-scann/<repo>/_intermediate.json` 读取（由 `cann-ops:scann-repo` 生成），无需手动配置。

**输出路径**：所有运行产物写到用户当前工作目录（CWD）的 `950-test/` 子目录：
- `CWD/950-test/run_state.json` — 算子跑测状态
- `CWD/950-test/logs/` — 每个算子的 build/install/run 日志
- `CWD/950-test/PHASE1_FINAL_REPORT.md` — 最终报告
...
激活后首先检查 `~/.config/cann-ops/config.json` 是否已配置各仓路径：

```bash
python3 scripts/config.py status
```

若输出有 `[未配置]` 条目，通过对话询问用户对应仓的本地路径，然后逐一写入：

```bash
python3 scripts/config.py set-repo <repo> <local-path>
```

路径配置一次即永久生效，无需用户手动设置环境变量。

**P1 — CANN 环境**：`ASCEND_HOME_PATH` 由 CANN toolkit 安装时自动设置，`scripts/utils.py:run_cmd()` 从中推导 `set_env.sh` 并自动 source，无需用户操作。若 `ASCEND_HOME_PATH` 未设置，说明 CANN toolkit 未安装，提示用户先安装。

**P2 — 硬编码 SOC**：所有脚本 `SOC = "ascend950"` 写死，不再参数化。
...
| 跑单算子 phase 1 | `python3 scripts/phase_examples.py --repo <name> --repo-path <path> --op <op>` |
...
**数据来源**（按优先级）：
1. `CWD/950-test/run_state.json` — 算子 × phase × status × duration_s（权威）
2. `CWD/950-test/logs/<repo>/<op>.phase{N}.{step}.log` — 真实错误信息摘录
3. `CWD/950-scann/<repo>/_intermediate.json` — 950 特性命中数据（与 scann-repo 共享）
4. `CWD/950-test/phase{N}_*_report.json` — 过程性 JSON
