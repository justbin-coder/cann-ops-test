...
一句话:把自己当成一个**只会照文档做、不会自己想办法**的新开发者,严格按文档一步步执行,看能不能跑起来。**默认出两份报告**:**忠实趟**(纯按文档卡在哪)+ **探索趟**(逐个补坑续跑看能否跑通)。

## ⚠ 本 skill 的反常之处(灵魂,不可违背)

其它 skill(尤其 `ops-test` 的 P6)鼓励**主动探索找 workaround**;**本 skill 的「忠实趟」恰恰相反——禁止任何文档以外的探索**。

> 因为:探索也许能把它跑起来,但那是**你**的本事,不是**文档**的本事。一旦你 source 了文档没让你 source 的环境、加了文档没写的 flag、改了文档里的错命令、清了缓存重试……你就**掩盖了文档的缺陷**。开发者照着这份文档**跑不起来**这一事实,必须先被忠实趟如实暴露。

**忠实趟**:只做文档明写的操作,文档没写的一律不做,文档错的照错跑,**卡住就停**——这是主结论。
**探索趟**(补充):从卡点起,**只把忠实趟自己提出的「修订建议」当补丁注入**(source/soc/网络这类有据的最小修复,**绝非任意 workaround**),逐个补坑续跑,看文档流程到底通不通、一共几个坑;碰到非文档原因(机器/设备)即止,**跑不通也没关系**。两趟合起来 = 文档质量的「下限 + 全貌」。
...
- 把「选定文档 + 声明的前提」落盘(供报告引用):
  ```bash
  cd skills/quickstart-check
  # 两趟各记一次 meta(忠实趟 / 探索趟各自子目录)
  python -m scripts.run_step --repo <repo>/faithful --meta --doc <文档相对仓根路径> \
    --prereq 'CANN 已装' --prereq '<文档声明的其它前提>'
  ```

> **默认出两份报告**:**忠实趟**(P3,纯按文档卡在哪)+ **探索趟**(P4,逐个补坑续跑看能否跑通)。两趟用两套子目录隔离:`--repo <repo>/faithful` 与 `--repo <repo>/explored`。

### P3 — 忠实趟(Pass 1):逐字执行 + 卡住即停 → 忠实报告

按 P1 顺序,对每个 `command` 步:**① 执行 → ② agent 看真实输出后判定**。用 `--repo <repo>/faithful`。

```bash
cd skills/quickstart-check
# ① 忠实执行(原样命令;绝不注入文档外 env/flag/source)
python -m scripts.run_step --repo <repo>/faithful --idx <n> --cwd <doc 指明的目录> \
  --cmd '<逐字命令>' --doc-quote '<文档原文>' --expected '<文档声称的期望>'
# ② agent 对照文档期望判定(verdict ∈ OK/FAIL/DOC_AMBIGUOUS/DOC_MISSING)
python -m scripts.run_step --repo <repo>/faithful --idx <n> --judge --verdict <verdict> \
...
**遇到第一个 blocker(`FAIL`/`DOC_AMBIGUOUS`/`DOC_MISSING`)→ 立即停**(开发者就是会在这卡住),记下卡点(`--defect` + `--fix`)。
**忠实趟铁律**:失败**不重试、不换参数、不加 flag/env、不 source、不找替代命令、不清缓存、不改文档命令、不做文档没写的任何动作**。即使「明知怎么修」也不修——那会掩盖文档缺陷。

出忠实报告:
```bash
python -m scripts.render_report --repo <repo>/faithful --kind faithful \
  --out <CWD>/cann-ops-report/doccheck/<repo>/faithful/REPORT.md
```

### P4 — 探索趟(Pass 2):补坑续跑 → 探索报告

目的:**忠实趟只告诉你卡在第一个坑;探索趟告诉你文档一共几个坑、补齐后流程到底通不通。** 用 `--repo <repo>/explored`,从头**带累积修复**重跑一遍。

- 对忠实趟暴露的每个**文档缺陷**,注入忠实趟为它写的「修订建议」(**只能是本 skill 提出的、有据的最小修复**:如 `source set_env.sh`、`--soc` 按芯片改、build 需要的 `http_proxy`),把注入内容用 `--injected-fix` 记进该步;然后续跑下一步,再遇 doc 缺陷再补再续。
  ```bash
  python -m scripts.run_step --repo <repo>/explored --idx <n> --cwd <dir> \
    --doc-quote '[假设文档补了 X] <文档原命令>' \
    --cmd '<注入修复 && 文档原命令>'
  python -m scripts.run_step --repo <repo>/explored --idx <n> --judge --verdict <verdict> \
    --injected-fix '<本步为绕过文档缺陷注入了什么:source/soc/proxy...>' [--defect ...] [--fix ...]
  ```
- **终止条件(任一)**:① 全流程跑通(✅);② 碰到**非文档原因**(机器/运行时,如 NPU `aclrtSetDevice` 错)——判 `FAIL`、`--defect` 注明「非文档缺陷」、收尾;**最终跑不通也没关系,可提前结束**;③ 某文档缺陷找不到「本 skill 提出的」明确修复 → 止于该步。
- **探索纪律**:只注入忠实趟已写进缺陷清单的修订建议,**不是任意 workaround**;非文档原因(设备/驱动/机器状态)不算文档坑,如实标注、不硬凑。

出探索报告:
```bash
python -m scripts.render_report --repo <repo>/explored --kind explored \
  --out <CWD>/cann-ops-report/doccheck/<repo>/explored/REPORT.md
```
...
> 失败/输出**必须是真实执行日志**;推断项标 `(推断)`,与真实摘录严格区分。
...
- ✗ 不修文档里的笔误 / 错误命令(逐字照跑,把错误当缺陷记下);文档歧义 / 缺步不替它补全或猜测(记为缺陷,卡住即停)。

**探索趟(P4)——有限放开,但守纪律**:
- ✓ 可注入修复**让文档流程往下走**,但**只能注入忠实趟已写进缺陷清单的「修订建议」**(source/soc/网络这类有据的最小修复),且每步用 `--injected-fix` 如实记录。
- ✗ 仍**不许任意 workaround**(改算子源码、换命令绕过、瞎试参数);非文档原因(机器/驱动/设备状态)**不算文档坑**,如实标注、不硬凑,**跑不通就收尾**。
...
## Output(产物:`cann-ops-report/doccheck/<repo>/`,**默认两份报告**)
...
| `injected_fix` | **探索趟**专用:本步为绕过文档缺陷实际注入了什么(忠实趟恒空) |
